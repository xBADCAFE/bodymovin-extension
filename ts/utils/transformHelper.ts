function exportTransform(layerInfo: any, data: any, frameRate: number): void {
  const bm_keyframeHelper = $.__bodymovin.bm_keyframeHelper;
  const settingsHelper = $.__bodymovin.bm_settingsHelper;

  const skipDefaultProperties = settingsHelper.shouldSkipDefaultProperties();

  if (!layerInfo.transform) {
    return;
  }
  const stretch = data.sr;
...
export const bm_transformHelper = {
